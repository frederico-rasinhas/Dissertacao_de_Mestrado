...
class VOSensor(ErrorStateSensorModel):
    def __init__(self):
        super().__init__("VO Sensor")
    ...
    def measure(self, filter, *args, **kwargs):
        ...
        dyaw = kwargs['dyaw']
        dt = kwargs['dt']
        inleirs_pose = kwargs['inliers_pose']
        inleirs_plane = kwargs['inliers_plane']
        ...
        if dt <= 1e-8:
            ...
            return z, 1e6 * np.eye(7)  # Very high noise → low confidence

        # === Dynamic noise model based on number of features ===
        N_total = inleirs_pose + inleirs_plane
        sigma = 1.0 / ((N_total*2)+ 1)
        print(f"[VO] sigma = {sigma}")
        ...
        # Body-frame velocity estimated from increments
        vB = np.array([[vx, vy, vz]]).T  # shape (3,1)

        # First call: use current orientation without applying incremental rotation
        if self.qOld is None:
            self.qOld = qt.as_quat_array(filter.stateParts("Orientation").ravel())
            z = np.concatenate((vB, filter.stateParts("Orientation")))  # shape (7,1)
            return z, 1000 * self.R  # low confidence in first measurement

        # Update orientation by applying incremental yaw rotation
        phi = np.array([0, 0, dyaw])  # Incremental orientation
        dq = qt.from_rotation_vector(phi)
        q = qt.as_float_array(self.qOld * dq).reshape(-1, 1)

        # Measurement: [body velocity; orientation]
        z = np.concatenate((vB, q))
    ...
    def estimate(self, filter):
        ...
        # Rotation from world → body frame (Rᵀ)
        qw, qx, qy, qz = q
        R = np.array([
