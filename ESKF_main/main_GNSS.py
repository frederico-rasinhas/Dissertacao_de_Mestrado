...
gnssSensor = GNSS_Sensor()     # GNSS sensor model
eskf = ErrorStateKalmanFilter(imuModel, gnssSensor)
...
# === Accelerometer and Gyroscope Bias Drift ===
imuModel.accBiasVar = 1e-8**2
imuModel.gyroBiasVar = 1e-8**2

# === Set GNSS Sensor Noise ===
gnssSensor.R = np.eye(3) * 1e-2**2  # Very optimistic GNSS (low variance)
...
for k, (ts, key, data) in enumerate(events, start=0):
    ...
    if key == 'imu':
        ...
        # Update step with GNSS measurements (lat, lon, alt, covariance)
