...
from LIDARSensor import LIDAR_Sensor
from GnssSensor import GNSS_Sensor

# Load data from rosbag + CSVs for VO and LiDAR
loader = DataLoader(
    bag_path="",
    vo_csv_path="",
    lidar_csv_path="",
...
lidarSensor= LIDAR_Sensor()      # LiDAR sensor
gnssSensor = GNSS_Sensor()       # GNSS sensor

# Instantiate ESKF with all available sensors
eskf = ErrorStateKalmanFilter(imuModel, voSensor, odoSensor, lidarSensor, gnssSensor)
...
odoSensor.R = np.eye(7) * 1e-4**2

# === Set GNSS Sensor Noise ===
gnssSensor.R = np.eye(3) * 1e-3**2  # GNSS covariance (moderate noise)
...
for k, (ts, key, data) in enumerate(events, start=0):
    ...
    # --- LiDAR update ---
        ...
        if t_prev_lidar is None:
            dt_lidar = 0.0
        else:
            dt_lidar = ts - t_prev_lidar
        eskf.update('LIDAR Sensor',
                    dx=data['dx'], dy=data['dy'], dyaw=data['dyaw'],
                    n_edges=data['n_edges'], n_flats=data['n_flats'],
                    dt=dt_lidar)
        t_prev_lidar = ts

    # --- GNSS update (only between 170s and 180s) ---
    elif key == 'gnss':
        if 170 <= ts - t0 <= 180:
