...
# Load data from rosbag + CSV for LiDAR poses
loader = DataLoader(
    bag_path="",
    lidar_csv_path=""
)
...
lidarSensor= LIDAR_Sensor()    # LiDAR sensor model
eskf = ErrorStateKalmanFilter(imuModel, lidarSensor)
...
# === Set LiDAR Sensor Noise (optional static tuning, otherwise dynamic) ===
# lidarSensor.R = np.eye(7) * 1e-4**2
...
for k, (ts, key, data) in enumerate(events, start=0):
    ...
    if key == 'imu':
        ...
        # Update step with LiDAR (incremental pose: dx, dy, dyaw + feature counts)
        eskf.update('LIDAR Sensor', dx=data['dx'], dy=data['dy'], dyaw=data['dyaw'],
                    n_edges=data['n_edges'], n_flats=data['n_flats'], dt=dt_lidar)
