...
# Instantiate ESKF with IMU + Odometry + GNSS
eskf = ErrorStateKalmanFilter(imuModel, odoSensor, gnssSensor)
...
# === Set Wheel Odometry Noise ===
odoSensor.R = np.eye(7) * 1e-3**2

# === Set GNSS Sensor Noise ===
gnssSensor.R = np.eye(3) * 1e-2**2  # Low covariance → "super-GPS" setting

# === Set initial uncertainty (error state covariance) ===
eskf.errStateCovParts("dPos", np.ones(3) * 1e1)
eskf.errStateCovParts("dVel", np.ones(3) * 1e0)
eskf.errStateCovParts("dTheta", np.ones(3) * 1e0)
eskf.errStateCovParts("dAccBias", np.ones(3) * 1e-1**2)
eskf.errStateCovParts("dGyroBias", np.ones(3) * 1e-4**2)

# Optional: manually set initial biases
...
# --- GNSS update window configuration ---
X_SECONDS = 10.0  # time window (first 10 s and last 10 s)

# Last GNSS timestamp (used for selective updates at the end of the dataset)
...
for k, (ts, key, data) in enumerate(events, start=0):
    ...
    # --- GNSS update (only during first and last X seconds) ---
    elif key == 'gnss':
        if ((ts - t0) <= X_SECONDS) or (t_last_gnss is not None and (t_last_gnss - ts) <= X_SECONDS):
            eskf.update('GNSS Sensor', lat=data['pos'][0], lon=data['pos'][1],
                        alt=data['pos'][2], cov=data['cov'])
            t_prev_gps = ts
...
# --- 3D Trajectory (top-down debug view) ---
